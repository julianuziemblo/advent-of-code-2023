...
fn parse_category(category: &str) -> Map {
    ...
    for range in ranges.into_iter() {
        let range = range.split_whitespace()
            .collect::<Vec<&str>>();
...
fn parse_input(input: String) -> (Vec<Seed>, Vec<Map>) {
    ...
    let seeds = categories[0].split_whitespace()
        .collect::<Vec<&str>>()[1..]
        .iter()
        .map(|seed|
            seed.trim()
...
fn lowest_location(seeds: &Vec<Seed>, maps: &Vec<Map>) -> Location {
...
fn get_ranges(seeds: &Vec<Seed>) -> Vec<(Range<Seed>, usize)> {
    ...
    for i in 0..(seeds.len() / 2) {
        ...
        ranges.push( (seeds[range_start_idx] .. seeds[range_start_idx] + seeds[range_len_idx], seeds[range_len_idx] as usize) );
    }
...
fn lowest_location_ranges(seeds: &Vec<Seed>, maps: &Vec<Map>) -> Location {
    ...
    for (i, range) in ranges.into_iter().enumerate() {
        ...
        println!("[{}/{ranges_len}] Calculating for seeds in range {range:#?} (length: {range_len:#?})", i+1);
...
fn main() {
    ...
    println!("Lowest location in ranges of seeds: {}", lowest_location_ranges(&seeds, &maps));
}
